...
var dbRequest = async (dbPath, sqlQuery) => {
  const dbQueryResult = (0, import_child_process.spawn)("sqlite3", [dbPath, sqlQuery, "-json"]);
  let result = "";
...
var annotationsRequest = async (dbPath, sqlQuery) => {
  const dbQueryResult = (0, import_child_process.spawn)("sqlite3", [dbPath, sqlQuery, "-json"]);
  let result = "";
  for await (const chunk of dbQueryResult.stdout) {
    result += chunk;
...
var BOOKS_DB_PATH = import_path.default.join(import_os.default.homedir(), "Library/Containers/com.apple.iBooksX/Data/Documents/BKLibrary/BKLibrary-1-091020131601.sqlite");
var HIGHLIGHTS_DB_PATH = import_path.default.join(import_os.default.homedir(), "Library/Containers/com.apple.iBooksX/Data/Documents/AEAnnotation/AEAnnotation_v10312011_1727_local.sqlite");
var BOOKS_LIBRARY_NAME = "ZBKLIBRARYASSET";
var HIGHLIGHTS_LIBRARY_NAME = "ZAEANNOTATION";
var BOOKS_LIBRARY_COLUMNS = [
  "ZASSETID",
  "ZTITLE",
  "ZAUTHOR",
  "ZGENRE",
...
var HIGHLIGHTS_LIBRARY_COLUMNS = [
  "ZANNOTATIONASSETID",
  "ZFUTUREPROOFING5",
...
// src/methods/getAnnotations.ts
var getAnnotations = async () => {
...
// src/utils/preserveNewlineIndentation.ts
var preserveNewlineIndentation = (textBlock) => {
  const stringWithNewLines = /\n+\s*/g;
  return stringWithNewLines.test(textBlock) ? textBlock.replace(stringWithNewLines, "\n") : textBlock;
};

// src/utils/removeTrailingSpaces.ts
var removeTrailingSpaces = (textBlock) => {
  const endLineSpaces = /\s+$/;
  return endLineSpaces.test(textBlock) ? textBlock.replace(endLineSpaces, "") : textBlock;
};

// src/methods/aggregateDetails.ts
var aggregateBookAndHighlightDetails = async () => {
  const books = await getBooks();
  const annotations = await getAnnotations();
  const resultingHighlights = books.reduce((highlights, book) => {
...
// src/utils/checkBookExistence.ts
var import_path2 = __toESM(require("path"));
var checkBookExistence = (bookTitle, vault, settings) => {
  const pathToBookFile = import_path2.default.join(settings.highlightsFolder, `${bookTitle}.md`);
  const doesBookFileExist = vault.getFileByPath(pathToBookFile);
  return doesBookFileExist ? true : false;
};

// src/search.ts
var IBookHighlightsPluginSuggestModal = class extends import_obsidian.SuggestModal {
  constructor(app, plugin) {
    super(app);
    this.plugin = plugin;
  }
};
var IBookHighlightsPluginSearchModal = class extends IBookHighlightsPluginSuggestModal {
  async getSuggestions(query) {
    try {
      ...
      new import_obsidian.Notice(`[${this.plugin.manifest.name}]:
Error importing highlights. Check console for details (\u2325 \u2318 I)`, 0);
  ...
  async onChooseSuggestion(item, event) {
...
var OverwriteBookModal = class extends import_obsidian.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian.Setting(contentEl).addButton((YesButton) => {
      YesButton.setButtonText("Yes").setCta().onClick(() => {
        bookToOverwrite ? this.plugin.saveHighlights.saveSingleBookHighlightsToVault([bookToOverwrite], false) : this.plugin.aggregateAndSaveHighlights();
        this.close();
      });
    }).addButton((NoButton) => {
...
// src/methods/saveHighlightsToVault.ts
var import_path4 = __toESM(require("path"));

// src/methods/renderHighlightsTemplate.ts
var Handlebars2 = __toESM(require_handlebars());
...
var calculateAppleDate = (date) => {
  import_dayjs.default.extend(import_utc.default);
  import_dayjs.default.extend(import_timezone.default);
  const timeZone = import_dayjs.default.tz.guess();
  const APPLE_EPOCH_START = new Date("2001-01-01").getTime();
  const dateInMilliseconds = date * 1e3;
  const calculatedDate = (0, import_dayjs.default)(APPLE_EPOCH_START).add(dateInMilliseconds, "ms").tz(timeZone || "UTC");
...
(() => {
  ...
  import_handlebars.default.registerHelper("dateFormat", (date, format) => {
...
var highlightLocationToNumber = (highlightLocation) => {
  return highlightLocation.slice(8, -1).split(",").slice(0, -1).join(",").match(/(?<!\[)[/:]\d+(?!\])/g).map((match) => parseInt(match.slice(1)));
};
var compareLocations = (firstLocation, secondLocation) => {
...
// src/utils/backupHighlights.ts
var import_path3 = __toESM(require("path"));
var BackupHighlights = class {
  constructor(vault, settings) {
    this.vault = vault;
    this.settings = settings;
  }
  async backupAllHighlights() {
    ...
    if (highlightsFolder) {
      ...
      if (highlightsFilesToBackup.length > 0) {
        ...
        for (const file of highlightsFilesToBackup) {
          const fileName = import_path3.default.basename(file);
          await this.vault.adapter.copy(file, import_path3.default.join(highlightsBackupFolder, fileName));
        }
  ...
  async backupSingleBookHighlights(bookTitle) {
    const bookFilePathToBackup = import_path3.default.join(this.settings.highlightsFolder, `${bookTitle}.md`);
    const vaultFile = this.vault.getFileByPath(bookFilePathToBackup);
    const backupBookTitle = `${bookTitle}-bk-${Date.now()}.md`;
    await this.vault.adapter.copy(vaultFile.path, import_path3.default.join(this.settings.highlightsFolder, backupBookTitle));
  }
};

// src/methods/saveHighlightsToVault.ts
var SaveHighlights = class {
  constructor(app, settings) {
    this.app = app;
    this.vault = this.app.vault;
    this.settings = settings;
  }
  async saveAllBooksHighlightsToVault(highlights) {
    ...
    for (const combinedHighlight of highlights) {
      ...
      const filePath = import_path4.default.join(this.settings.highlightsFolder, `${combinedHighlight.bookTitle}.md`);
      await this.createNewBookFile(filePath, renderedTemplate);
    }
  }
  async saveSingleBookHighlightsToVault(highlights, shouldCreateFile) {
    const highlightsFolderPath = this.vault.getFolderByPath(
      this.settings.highlightsFolder
    );
    if (!highlightsFolderPath) {
  ...
  async modifyExistingBookFile(file, data) {
    await this.vault.modify(
      file,
      data
    );
  }
  async createNewBookFile(filePath, data) {
...
// src/template.ts
var defaultTemplate = `Title:: \u{1F4D5} {{{bookTitle}}}
Author:: {{{bookAuthor}}}
...
- \u{1F4D6} Chapter:: {{#if chapter}}{{{chapter}}}{{else}}N/A{{/if}}
- \u{1F516} Context:: {{#if contextualText}}{{{contextualText}}}{{else}}N/A{{/if}}
- \u{1F3AF} Highlight:: {{{highlight}}}
- \u{1F4DD} Note:: {{#if note}}{{{note}}}{{else}}N/A{{/if}}
- \u{1F4D9} Highlight Link:: {{#if highlightLocation}}[Apple Books Highlight Link](ibooks://assetid/{{../bookId}}#{{highlightLocation}}){{else}}N/A{{/if}}

{{/each}}
`;
var template_default = defaultTemplate;

// src/settings.ts
var AppleBooksHighlightsImportPluginSettings = class {
  constructor() {
    this.highlightsFolder = "ibooks-highlights";
    this.backup = false;
    this.importOnStart = false;
    this.highlightsSortingCriterion = "creationDateOldToNew" /* CreationDateOldToNew */;
    this.template = template_default;
  }
};
var IBookHighlightsSettingTab = class extends import_obsidian2.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  display() {
    ...
    containerEl.empty();
    const folder = new import_obsidian2.Setting(containerEl).setName("Highlights folder").setDesc("A folder (within the root of your vault) where you want to save imported highlights").setClass("ibooks-highlights-folder");
    folder.addText((text) => text.setPlaceholder("Folder to save highlights").setValue(this.plugin.settings.highlightsFolder).onChange(async (value) => {
      if (!value) {
    ...
    new import_obsidian2.Setting(containerEl).setName("Import highlights on start").setDesc("Import all highlights from all your books when Obsidian starts").addToggle((toggle) => {
      toggle.setValue(this.plugin.settings.importOnStart).onChange(async (value) => {
        this.plugin.settings.importOnStart = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian2.Setting(containerEl).setName("Backup highlights").setDesc(createFragment((el) => {
      ...
      toggle.setValue(this.plugin.settings.backup).onChange(async (value) => {
        if (!value) {
          new import_obsidian2.Notice("Disabling backups imposes a risk of data loss. Please use with caution.", 0);
        }
        this.plugin.settings.backup = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian2.Setting(containerEl).setName("Highlights sorting criterion").setDesc("Sort highlights by a specific criterion. Default: By creation date (from oldest to newest)").setClass("ibooks-highlights-sorting").addDropdown((dropdown) => {
      const options = {
        ["creationDateOldToNew" /* CreationDateOldToNew */]: "By creation date (from oldest to newest)",
        ["creationDateNewToOld" /* CreationDateNewToOld */]: "By creation date (from newest to oldest)",
        ["lastModifiedDateOldToNew" /* LastModifiedDateOldToNew */]: "By modification date (from oldest to newest)",
        ["lastModifiedDateNewToOld" /* LastModifiedDateNewToOld */]: "By modification date (from newest to oldest)",
        ["book" /* Book */]: "By location in a book"
      };
      dropdown.addOptions(options).setValue(this.plugin.settings.highlightsSortingCriterion).onChange(
        async (value) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Template").setDesc("Template for highlight files").setClass("ibooks-highlights-template").addTextArea((text) => {
      text.setPlaceholder("Template").setValue(this.plugin.settings.template).onChange(async (value) => {
        const valueToSet = value === "" ? template_default : value;
        this.plugin.settings.template = valueToSet;
        await this.plugin.saveSettings();
      });
      return text;
    });
    new import_obsidian2.Setting(containerEl).setName("Reset template").setDesc("Reset template to default").addButton((button) => {
      button.setButtonText("Reset template").onClick(async () => {
        this.plugin.settings.template = template_default;
        await this.plugin.saveSettings();
        this.display();
      });
    });
    containerEl.createEl("hr");
    containerEl.createEl("small", { text: "Created by ", cls: "credits" }).createEl("a", {
      text: "bandantonio",
...
// main.ts
var IBookHighlightsPlugin = class extends import_obsidian3.Plugin {
  async onload() {
    ...
    this.addRibbonIcon("book-open", this.manifest.name, async () => {
      try {
        this.settings.backup ? await this.aggregateAndSaveHighlights().then(() => {
          new import_obsidian3.Notice("Apple Books highlights imported successfully");
        }).catch((error) => {
          new import_obsidian3.Notice(`[${this.manifest.name}]:
Error importing highlights. Check console for details (\u2325 \u2318 I)`, 0);
...
Error importing highlights. Check console for details (\u2325 \u2318 I)`, 0);
  ...
  async aggregateAndSaveHighlights() {
